...
Courier offers caching and retry mechanics, inter-request dependencies ("fetch
the OAuth token before hitting this API"), and deferred parameterization - all
of which can be tuned to a priori information about individual endpoints.
...
(def spotify-token
  {::http/request-fn
   ...
   ::http/cache-for-fn #(* 1000 (-> % :body :expires_in))})
...
The `result` map returned from `http/request` differs from an HTTP response.
A Courier request can result in multiple request/response pairs (e.g. if retries
are required), and the resulting map contains all information about the result
and all attempts at acquiring it. `::http/data` contains the body of the
response.
...
```clj
cjohansen/courier {:mvn/version "0.2020.09.26"}
```

With Leiningen:

```clj
[cjohansen/courier "0.2020.09.26"]
```
...
- `:courier.res/cache-status` - A map describing the cache status of the data
...
- `:courier.attempt/req` - The request map
- `:courier.attempt/res` - The full response
- `:courier.attempt/retryable?` - A boolean
- `:courier.attempt/cacheable?` - A boolean
...
which will receive both the request and the response:
...
```clj
(defprotocol Cache
  (make-key [_ {:keys [spec params request cache-params]}])
  (get [_ k])
  (set [_ k res]))
```

`make-key` will be called to generate a unique cache key for the request. It is
passed a map that always contains the full map passed to `courier.http/request`
as `spec`.

If `:courier.http/params` is set, the map will also contain a map of `params`
with all the specified keys and their resulting values, as well as the `request`
returned from `:courier.http/request-fn`.

If `:courier.http/cache-params` is set, `:params` and `:request` will _not_ be
available, instead the map will contain a map of `cache-params` with the
specified keys and their resulting values.

The return value from `make-key` will be passed as `k` to the other cache
functions.

`get` attempts to load a cached response for the request. It will be passed the
cache key produced by `make-key` as `k`. If this function returns a non-nil
value, the value will be used as the response, and `set` will never be called.

If the value does not exist in the cache, the request will be made, and if
successful, `set` will be called with the cache key and the response.
...
Cache keys can be used in place of the full request to make more efficient use
...
cache key from the `:courier.http/cache-keys` only, Courier will omit the token
...
With `:courier.http/cache-params` in place, `courier.cache/make-key` won't
receive the full request, only the spec and the cache parameters (the playlist
ID). The `:courier.http/request-fn` can be used to identify the request, but it
usually won't do so in a human-friendly manner. A better approach is to include
`:courier.http/id` in the spec, which will be used by most cache implementations
to construct a short, human-friendly cache key:
...
the playlist with id `3b5045a0-05fc-4d7f-8b61-9c6d37ab90e6` under the following
key:
